...
# macOS
fdesetup status
# 期望形如: FileVault is On.  /  FileVault is Off.
...
|---|---|
| 没开 | 这条不影响你，跳过 |
| **开了** | ⚠️ **断电之后，机器在有人解锁之前不会回到可用状态。** 相册不备份、组网工具不上线、常驻 harness 不在——**而且不报错。** 停在哪一步、有没有远程解锁的例外，见 [`ops/boot-persistence.md`](ops/boot-persistence.md#全盘加密会限制自恢复能力) |
...
| **改不了**（不少运营商光猫锁了这些选项） | ⚠️ **"全屋"这条路直接不通**，只能逐台手动指网关，或者换一台自己的路由器做主路由。**这是要先问的，不是做到一半才发现的** |
