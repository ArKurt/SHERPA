...
> 本附录合并自 `reviews/sourcing-S1-codex.md`、`reviews/sourcing-S2-zcode.md`
> 与 `reviews/sourcing-S3-claude.md`。三份报告的共同基线为 `6f593fa`；
...
- **强度**：`[实测]` + `[推理]`
- **出处/依据**：无单一上游出处；推理起点见“原文/取证说明”。
- **原文/取证说明**：无逐字上游原文；S3 的证据是单次现场观察加定时机制差异推理，且未记录系统/版本。
- **手册怎么说**：低功耗和常驻可用是一对矛盾，没有两全的配置。
- **是否需改写**：**是** —— 建议：「睡眠期间错过的任务是否补跑取决于调度器及配置；例如 systemd timer 的 `Persistent=true` 可补跑，不能写成跨平台必然不补跑。」
- **来源报告**：`S3`
