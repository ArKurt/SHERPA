...
check_redaction() {
  ...
  hits=$(grep -rEn '100\.(6[4-9]|[7-9][0-9]|1[01][0-9]|12[0-7])\.[0-9]+\.[0-9]+' --include='*.md' . 2>/dev/null \
         | grep -v '100\.64\.0\.0/10' | grep -v '100\.100\.100\.100' || true)
  ...
  # 订阅 / 凭证关键词
  # 只匹配真的带内容的链接（裸 scheme 是格式说明，不是真链接）
  ...
  #
  # ⚠️ 词表【不在这个仓库里】。放在 scripts/.denylist（已 gitignore）。
  #    原因：把私有主机名写进检查器源码，等于用检查器把它们公开了——
  #    而检查器还排除自己，所以它永远不会因此变红。
  #    这条是公开前审查查出来的，见 scripts/.denylist.example。
  #    （.denylist 自身不会被扫到：下面的 grep 只看 .md/.html/.py/.sh/.json）
  local denylist="$(dirname "$0")/.denylist"
  if [ -f "$denylist" ]; then
    local words
    words=$(grep -vE '^\s*(#|$)' "$denylist" | paste -sd'|')
    if [ -n "$words" ]; then
      hits=$(grep -rniE "\\b($words)\\b" \
             --include='*.md' --include='*.html' --include='*.py' --include='*.sh' --include='*.json' . 2>/dev/null || true)
      [ -z "$hits" ] && ok "无内部主机名" || { bad "发现内部主机名"; echo "$hits" | head -10; }
    else
      ok "无内部主机名（词表为空）"
    fi
  else
    printf '  \033[33m注意\033[0m 没有 scripts/.denylist —— 跳过内部主机名检查。\n'
    printf '       这是给仓库作者用的私有词表，照 .denylist.example 建一份。\n'
  fi

  # 裸写的真实域名（不在 URL 里的），仍按后缀拦
  hits=$(grep -rEn '(^|[^/A-Za-z0-9._-])[a-z0-9-]+\.(uk|xyz|top)\b' --include='*.md' . 2>/dev/null || true)
