...
```sh
scripts/wizard.py          # 起本地服务、开浏览器；用户答完，清单从 stdout 回到你手上
```

它只监听 `127.0.0.1`，不联网。用户在浏览器里答三轮问题，**你不需要让他手工复制粘贴**。
无头环境加 `--no-browser`，把打印出来的地址给用户。

清单开头有一段机器可读的 frontmatter：

```yaml
vector:            # 配置向量，null 表示未定
needs_probe:       # 【机器能查的】—— 先实测，别猜
needs_decision:    # 【要读手册判据才能定的】—— 读对应的层
```

**这两类不一样，别混。** 前者跑命令就有答案，后者要读手册、有时还要问用户。

如果有清单，**它是你的起点**。那是用户在 `wizard.html` 里答完问卷生成的，包含：

- 用户的原话回答（设备、目标、约束）
- 一个**部分待实测**的配置向量 —— `⟨尖括号⟩` 里的是问卷问不出来的，**不要猜**
- 用户回答之间已知的冲突（清单只指路，由你读手册去裁决）
- 必须停下来问用户的清单
- 一条按他的选择裁剪过的施工序

**清单不含任何技术判据**，判据全在本手册。清单告诉你「用户要什么」，
手册告诉你「那意味着什么」。两者冲突时以手册为准，并告诉用户。
...
**只认模块声明的验收，且必须从客户端跑。** 两种模块的验收写在不同地方：

| 模块 | 验收在哪 |
|---|---|
| `services/` · `blueprints/` | frontmatter 的 `verify` 字段 |
| `layers/` | 该层的 **`## 验收`** 小节 |

⚠️ **内核状态、服务日志、控制接口的输出都不算验收。**
它们能证明"组件加载了"，不能证明"客户端能用"。那些是**出问题时的取证手段**。

⚠️ **没有终端的设备（电视、机顶盒、游戏机）跑不了 `curl`。**
对这类设备，验收是"用它自己的 App 访问，观察结果"——
见 [层 7 的验收](layers/7-lan-addressing.md#验收)。
**不要因为跑不了命令就退回自己的机器测一遍然后宣布通过。**
