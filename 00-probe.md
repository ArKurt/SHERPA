...
- 定时任务（备份、清理）→ **可能整段丢掉**。会不会补跑取决于调度器：
  `cron` 跳过，macOS `launchd` 会在唤醒时补但多次错过合并成一次，
  `systemd` timer 默认跳过。**别假设它会自己追上来** →
  [`ops/boot-persistence.md`](ops/boot-persistence.md#睡过去错过的定时任务会不会补跑)
