...
这条规则来自一个真实教训——完整案例见
[层 3 · 坑一](../pitfalls/proxy-dns.md#裸核模式核能出海客户端全断)。
...
| 值 | 含义 |
|---|---|
| `anytime`（缺省） | 前置条件满足就能装 |
| `last` | **要等别的服务先就位**，否则装了也没用 |

目前只有导航面板是 `last` —— 没有别的服务，就没有导航的对象。

> ⚠️ **这不是"重要程度"，是时序。** 本字段曾经是 `priority: P0/P1/P2`，
> 那原本表示**写作顺序**（先写哪一批）。内容写完之后那个含义就作废了，
> 但它被向导拿去当展示排序，于是"我先写谁"被当成了"用户先看到谁"。
> 换成 `install_when` 就是为了消除这种语义漂移：
> **字段名说什么，它就只表示什么。**
